...
- [cargo-info](https://gitlab.com/imp/cargo-info) — please don't make me have to visit crates.io ever again
- [cargo-make](https://sagiegurari.github.io/cargo-make/), [cargo-script](https://github.com/DanielKeep/cargo-script), [cargo-x](https://github.com/liuchong/cargo-x), [tinyrick](https://github.com/mcandre/tinyrick), [just](https://github.com/casey/just) — not-makefile build task tools (questionable but)
...
- [criterion](https://docs.rs/criterion/0.2.11/criterion/) — a fancy and comprehensive benchmarking framework
...
- [rustversion](https://github.com/dtolnay/rustversion) — `#[cfg(..)]` for rustc versions
...
- [nushell](https://github.com/nushell/nushell) — powershell but not awful maybe
...
Most unstable rustdoc features can be considered usable because you can `#[cfg_attr(feature = "unstable_docs", ..)]` them out for stable while still supporting [docs.rs via metadata](https://docs.rs/about), which uses unstable rust to build everything.

- [external docs](https://doc.rust-lang.org/nightly/unstable-book/language-features/external-doc.html) — include docs and READMEs to avoid duplication or out-of-sync examples
- [doc(cfg)](https://doc.rust-lang.org/nightly/unstable-book/language-features/doc-cfg.html) — document an item as conditional on a particular condition or feature flag
- [alias](https://doc.rust-lang.org/nightly/unstable-book/language-features/doc-alias.html) — search aliases help with wrapper crates or FFI
- [masked](https://doc.rust-lang.org/nightly/unstable-book/language-features/doc-masked.html) — a more powerful `#[doc(hidden)]` for imported traits
- [spotlight](https://doc.rust-lang.org/nightly/unstable-book/language-features/doc-spotlight.html) — important traits can be highlighted when functions return types that impl them
- [intra-doc links](https://github.com/rust-lang/rust/issues/63305) — link to types by name
  - unfortunately there isn't a way to gate this without duplicating all of your docs with links, so documentation generated by a stable rustdoc will contain broken links
- [`doc_cfg`](https://github.com/arcnmx/doc-cfg) — aims to be a macro that can conditionally apply unstable rustdoc features with minimal boilerplate
